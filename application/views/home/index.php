...
.matka-result h4 {
  ...
  <style type="text/css">
    .satta-result{}.satta-result h4{font-size:22px;background-color:transparent;color:#1a237e;text-shadow:1px 1px #d9d9d9}.satta-result h5{margin:0;font-size:22px;background-color:transparent;color:#880e4f;text-shadow:1px 1px #0000001f;line-height:1}.satta-result h6{color:#7a028d;font-size:15px;padding:2px 0;text-shadow:1px 1px 2px #c4c4c4;margin-bottom:0}.satta-result div{padding:3px}.satta-result div:last-child{border-bottom-width:0}
    
    .result_timing,.result_timing_right{position:absolute;color:#7a028d;font-size:15px;padding:2px 0;transform:translateY(-158%);border-bottom:none!important}.result_timing{left:14px;right:auto}.result_timing_right{right:14px;left:auto}.btn_chart{border:2px solid #ff006c;background-color:#ff006c;color:#fff;padding:3px 7px;border-radius:8px 0;box-shadow: 0px 0px 1px #000000d6;font-size:12px;margin:2px 0 -1px;display:block;transition:all .3s;text-shadow:1px 1px 2px #222;margin-top:5px}.fg-div h4{border-radius:4px;background:#ff006c;color:#fff;text-shadow:1px 1px 0 #444;letter-spacing:1px;font-size:24px;padding:4px 4px 4px;margin-bottom:2px}.mb-1{margin-bottom:5px}.p-1{padding:5px 10px 8px}.bdr{border:2px solid #eb008b;border-radius:10px 0 10px 10px}.fg-div h6{font-size:22px;background-color:transparent;color:#1a237e;text-shadow:1px 1px #d9d9d9}.fg-div h5{font-size:22px;color:#880e4f;text-shadow:1px 1px 2px #ffe2c6}.fg-div .fg-p1{font-size:22px;text-shadow:1px 1px 2px #ffe2c6}.fg-div .fg-c1>div{border:2px solid #eb008b;border-radius:10px 0 10px 10px;margin:5px 0;padding:5px}.fg-div .fg-c1 .fg-p2{font-size:22px;color:#880e4f;text-shadow:1px 1px 2px #ffe2c6;border-radius:4px;background:#ff006c;color:#fff;text-shadow:1px 1px 0 #444;letter-spacing:1px;font-size:24px;padding:4px 4px 4px;margin-bottom:2px}.fg-div .fg-c1 .fg-p3{font-size:22px;color:#080808;text-shadow:1px 1px 2px #ffe2c6}.fg-div .fg-c1 .fg-p4{font-size:20px}</style>
...
<!-- <div class="mrque-div" style="display: flex;padding: 5px;align-items: center;"> -->
  <img src="<?php echo base_url();?>images/test6.png" alt="dpboss net LAXMI_PICTURE" width="90" height="68">
   ...
   </div><!-- satta matka text --><div style="border: 0px solid #eb008b;border-radius: 10px 0 10px 10px;margin-bottom: 2px;">    
  

<div>
<!--<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-9865019318636446"-->
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <div class="row gold-payment-type lazy" style="display: block;">
        <div>
          ...
          <div class="head head2" id="pc" ><h2 style="background-color:#342e36; font-size: 15px; color: yellow;" >!!! MATKA LIVE GAME RESULTS !!!</h2></div>
          <?php
...
<button id="openPopupBtn"
        style="position:fixed; bottom:20px; right:20px; background:#ed557b; color:#fff; border:none;
               border-radius:50px; padding:12px 18px; font-size:16px; cursor:pointer; box-shadow:0 2px 8px rgba(0,0,0,0.3);">
  📢 Promo
</button>

<!-- Popup -->
<div id="promoBox" style="display:none; position:fixed; top:20%; left:50%; transform:translateX(-50%);
                          z-index:9999; max-width:420px; width:92%; background:#ff006c; padding:20px;
                          border-radius:10px; color:170303; box-shadow:0 6px 24px rgba(0,0,0,0.5);">
  <span onclick="closePopup()" style="float:right; cursor:pointer; font-size:22px; font-weight:bold;">&times;</span>
  <h3 style="margin-top:0; text-align:center;">🎯 Submit Your Info</h3>

  <form id="marketingForm" style="text-align:center;">
    <input type="text" name="fullname" placeholder="Full Name" required
           style="width:92%; margin:8px 0; padding:10px; border-radius:6px; border:none;"><br>
    <input type="email" name="email" placeholder="Email" required
           style="width:92%; margin:8px 0; padding:10px; border-radius:6px; border:none;"><br>
    <input type="text" name="mobile_no" placeholder="Mobile No" required
           style="width:92%; margin:8px 0; padding:10px; border-radius:6px; border:none;"><br>
    <!-- site_id will be filled when popup opens -->
    <input type="hidden" name="site_id" id="site_id" value="">
    <button type="submit" style="width:92%; padding:10px; border-radius:6px; border:none; background:#fff; color:#ff006c; font-weight:bold;">
      ✅ Submit
    </button>
  </form>
</div>
...
<script>
const promoBtn = document.getElementById('openPopupBtn');
const promoBox = document.getElementById('promoBox');
const marketingForm = document.getElementById('marketingForm');
const siteIdInput = document.getElementById('site_id');

function openPopup() {
  // call marketing/popup to get redirect_url and site_id
  fetch("<?= site_url('marketing/popup') ?>")
    .then(resp => {
      // helpful debug: log status and text if not JSON
    ...
    .then(data => {
      ...
      // store site_id if provided (so submit() can send it)
      if (data.site_id) siteIdInput.value = data.site_id;

      // show popup
      promoBox.style.display = 'block';
    })
    .catch(err => {
      console.error('Error calling popup API:', err);
      // still open popup but without site_id; submit will use default.
      promoBox.style.display = 'block';
    });
}
...
promoBtn.addEventListener('click', openPopup);

// On submit: send form as FormData to marketing/submit
marketingForm.addEventListener('submit', function(e) {
  e.preventDefault();

  const formData = new FormData(marketingForm);

  fetch("<?= site_url('marketing/submit') ?>", {
    method: 'POST',
    body: formData
  })
  .then(resp => {
  ...
  .then(data => {
    ...
    if (data.status === 'success' && data.redirect_url) {
      ...
      // fallback redirect
      alert('Something went wrong, redirecting to default');
      // window.location.href = data.redirect_url || 'https://www.playmaxx.club';
    }
  })
  .catch(err => {
    console.error('Submit error:', err);
    // fallback
    // window.location.href = 'https://www.playmaxx.club';
  });
});
</script>
...
| KALYAN MATKA RESULT  | MAIN MATKA | madhur morning | madhur day | madhur night  </p></h2>
...
$date3567 = strtotime ($midnightsunset);
  ...
  <div <?php  if(($row->higlightbazar)){ ?>   
     ...
     <?php if($row->higlightnotice == 1){ ?>
      <h5>  <marquee STYLE="color:red;font-weight:bold; font-size:100%;font-style:normal"><B>
    	    	             <?php echo  $row->Notice; ?> </h5>
    	    	             </B>
    	    	             </marquee>
     <?php }else{?>
     
     <h5>  <marquee STYLE="color:black;font-weight:bold; font-size:100%;"><B>
    	    	             <?php echo  $row->Notice; ?> </h5>
    	    	             </B>
    	    	             </marquee><?php } ?>
    	    	             <center>
    <h4><font size=5px"><b><?php echo $row->bazar_name; ?></b></font></h4>
    <h5 class="blinking" ><font size="5px"><b style="font-size: 30px;"><?php echo $gameResult ?></b></font></h5>
    <h6><font size="4px"><?php echo date('h:i a', strtotime($row->open_time)); ?>   &nbsp;&nbsp; <?php echo date('h:i a', strtotime($row->close_time));?></font></h6>
    
    </center>
    <div class="result_timing">
        <a href="<?php echo base_url();?>home/jodi_chart/<?php  echo $row->id;  ?>/<?php echo $row->bazar_name;?>" class="btn_chart"><font size="3px">Jodi</font></a>
    </div>
    <div class="result_timing_right">
      <a href="<?php echo base_url();?>home/panel_chart/<?php  echo $row->id;  ?>/<?php echo $row->bazar_name;?>" class="btn_chart"><font size="3px">Panel</font></a>
    </div>
  </div>
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
  ...
  <div class="head" id="jc">
    <h2 style="background-color:#342e36; font-size: 22px; color: yellow;">
      ALL MATKA JODI CHART RECORDS
    </h2>
  </div>
  <ul>
    <?php
    if($fetch_data->num_rows() > 0)
    {
      foreach($fetch_data->result() as $row)
      {
          // Convert bazar name into slug (for clean URL)
          $bazar_slug = strtolower(str_replace(' ', '-', $row->bazar_name));
    ?>
        <a href="<?php echo base_url('jodi-chart/'.$bazar_slug); ?>">
          <li><p><?php echo $row->bazar_name; ?></p></li>
        </a>
    <?php
      }
    }
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
  ...
  <ul>
    ...
    if($fetch_data->num_rows() > 0)
    {
      foreach($fetch_data->result() as $row)
      {
          // Convert bazar name to SEO-friendly slug
          $bazar_slug = strtolower(str_replace(' ', '-', $row->bazar_name));
    ?>
        <a href="<?php echo base_url('panel-chart/'.$bazar_slug); ?>">
          <li><p><?php echo $row->bazar_name; ?></p></li>
        </a>
    <?php
      }
...
<div class="head" id="jc" ><h2 style="background-color:#342e36; font-size: 22px; color: yellow;">!!  TIME TABLE !!</h2></div>
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
