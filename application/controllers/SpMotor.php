...
class SpMotor extends CI_Controller {
    ...
    function __construct(){
       ...
       $access_level=$this->session->userdata('access_level');
        if(!$customer_logged_in and $access_level!='5' ){
	...
	public function index()
	{
		...
		$submit=$this->input->post('buy');
		if(isset($submit))
		{
		 	$this->form_validation->set_rules('aakdano','aakdano', 'required');
		 	$this->form_validation->set_rules('panaamount','panaamount', 'required');
				...
				if($this->User_Model->userTransactionStatus($this->input->post('client'))){
					if($this->Bazar_Model->checkBazarTime($this->input->post('bazar'),$this->input->post('bazar_cat'))){
						...
						if($this->db->insert('user_passbook',$passbookData)){
						   ...
						   $data['messagesuccess']='Sp Motor Purchase Done';  
					}
				}else{
		...
		$this->load->view('user/spmotor',$data);
