...
class Bazar extends CI_Controller {
    	...
    	public function addbazar()
	{
      ...
      $this->form_validation->set_rules('openblocktime', 'Open block time', 'trim|required');
       ...
       $this->form_validation->set_rules('notice', 'notice', 'trim');
		...
		if ($this->form_validation->run() == TRUE) {
				...
				$notice=$this->input->post('notice');
					...
					$timeupdate =$this->input->post('timeupdate');
				if($timeupdate =='on'){
				    $timeupdate = 1;
				}else{
				    $timeupdate =0;
				}
				$editresult =$this->input->post('editresult');
				if($editresult =='on'){
				    $editresult = 1;
				}else{
				    $editresult =0;
				}
				$noticeupdate =$this->input->post('noticeupdate');
				if($noticeupdate =='on'){
				    $noticeupdate = 1;
				}else{
				    $noticeupdate =0;
				}
				$higlightbazar =$this->input->post('higlightbazar');
				if($higlightbazar =='on'){
				    $higlightbazar = 1;
				}else{
				    $higlightbazar =0;
				}
				$listbazar =$this->input->post('listbazar');
				if($listbazar =='on'){
				    $listbazar = 1;
				}else{
				    $listbazar =0;
