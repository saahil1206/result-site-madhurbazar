...
error_reporting(0);
class Admintriple extends CI_Controller {
	...
	public function index()
	{
 	...
 	$custid=$this->input->post('client');
				...
				$AakdaData=array(
				 ...
				 $date4,
				 $this->input->post('single0'),
                 $this->input->post('single1'),
                 $this->input->post('single2'),
                 $this->input->post('single3'),
                 $this->input->post('single4'),
                 $this->input->post('single5'),
                 $this->input->post('single6'),
                 $this->input->post('single7'),
                 $this->input->post('single8'),
				 $this->input->post('single9'),
				...
				$doubledetails=$this->Transaction_Model->generatequery('game_tripple_pana',$AakdaData);
				...
				$passbookData = array(
					  ...
					  'description'=>'Ticket purchase of TRIPPLE patti'.$ticketId,
					  'date'=>$date4,
					  'createdby'=>(string)$_SESSION['username'],
				...
				if($this->User_Model->userTransactionStatus($this->input->post('client'))){
					if($this->Bazar_Model->checkBazarTime($this->input->post('bazar'),$this->input->post('bazar_cat'))){
						...
						if($this->db->insert('user_passbook',$passbookData)){
						   ...
						   $data['messagesuccess']='Tripple patti Purchase Done';  
					}
				}else{
		...
		$this->load->view('admin/tripple',$data);
