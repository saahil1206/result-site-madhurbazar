...
<button type="button" class="btn btn-danger" data-dismiss="modal"><i class="icon-close"></i></button>
	...
	?></td>
									...
									<td>
										<div class="btn-group btn-group-sm">
										    <?PHP
										     if($row->GAME_Group_name !='OPERATOR'){
										    ?>
										...
										data-username="<?php  echo $row->username;?>">
											...
											ST
												...
												<?php if($row->deleted_at == NULL) { ?>
													<a onclick="delete_user(<?php echo $row->id ?>)" href="javascript:void(0);">
														<button type="button" class="btn btn-warning btn-lg"style="padding: 5px;">
															DELETE
														</button>
													</a>
								 ...
								 <?PHP
											    ...
											    ?>				
											<a href="javascript:void(0);" class="datestatus" 
										data-id="<?php  echo $row->id;?>" 
									...
									&nbsp;&nbsp;
									<?php
									if($row->searchField == 0){
									?>
									<a href="updateSearchStatus/<?php  echo $row->id;?>/1">
											<button type="button" class="btn btn-danger btn-lg"style="padding: 5px;">
										S-DEACT
											</button>
								    </a>			
									<?php
									...
									?>
									<a href="updateSearchStatus/<?php  echo $row->id;?>/0">
									    
										<button type="button" class="btn btn-success btn-lg"style="padding: 5px;">
										S-ACTIVT
									    </button>
									</a>
									<?php
									...
									if($row->updateTime == 0){
									?>
									<a href="updatetimeStatus/<?php  echo $row->id;?>/1">
											<button type="button" class="btn btn-danger btn-lg"style="padding: 5px;">
										UT-DEACT
											</button>
								    </a>			
									...
									?>
									<a href="updatetimeStatus/<?php  echo $row->id;?>/0">
									...
									if($row->aeresult == 0){
									?>
									<a href="updateaddeditStatus/<?php  echo $row->id;?>/1">
											<button type="button" class="btn btn-danger btn-lg"style="padding: 5px;">
										AE-DEACT
											</button>
								    </a>			
									...
									?>
									<a href="updateaddeditStatus/<?php  echo $row->id;?>/0">
	...
	<div class="modal fade bd-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
		...
		<form method="post" action="<?php  echo base_url(); ?>Account/changeDatestatus">
		<div class="modal fade" id="datestatus" tabindex="-1" role="dialog" aria-labelledby="basicModalLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="basicModalLabel" style=" font-size:15px;font-weight:bold;">User EXPIRY SETTING</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
						</button>
					</div>
					<div class="modal-body">
						...
						<div class="form-group">
						    <label class="form-label" for="startdate" style=" font-size:15px;font-weight:bold;">start Date</label>
							<input type="date" required class="form-control" name="startdate"  id="startdate">
							
						</div> <br>
						<div class="form-group">
						    <label class="form-label" for="enddate"   style=" font-size:15px;font-weight:bold;">End Date</label>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-warning" data-dismiss="modal"><i class="icon-reply"></i>&nbsp;Close</button>
							<input  	type="button" class="btn btn-warning" data-toggle="modal" data-target=".bd-example-modal-sm2" value="Save">
	...
	<div class="modal fade bd-example-modal-sm2" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel2" aria-hidden="true">
										<div class="modal-dialog modal-sm">
											<div class="modal-content">
												<div class="modal-header">
													<h5 class="modal-title"  id="mySmallModalLabel2">Are You Sure You Want to Submit ?</h5>
													<button type="button" class="close" data-dismiss="modal" aria-label="Close">
														<span aria-hidden="true">&times;</span>
													</button>
												</div>
												...
												<div class="modal-footer">
														<button type="button" class="btn btn-warning" data-dismiss="modal"><i class="icon-reply"></i>&nbsp;No</button>
							<input  type="submit" class="btn btn-primary" value="Yes" name="updateDate">
												</div>
											</div>
									  </div>
									</div>
	...
	function delete_user(id){
		 $.ajax({
            url:"<?php echo base_url();?>Account/deleteUser",
            type: "POST",
			data: { 'id': id },
            dataType:'html',
...
$('.limitData').on('click', function(){
	...
	//$('[name="blockedstatus"]').val(blockedstatus);
		...
		$('.datestatus').on('click', function(){
	
	var id = $(this).data('id');
	var username = $(this).data('username');
		var startdate = $(this).data('startdate');
		var enddate = $(this).data('enddate');
	$('#datestatus').modal('show');
	$('[name="id_edit"]').val(id);
	$('[name="clientname"]').val(username);
